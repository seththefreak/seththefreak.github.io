function TabDados({ char }) {
  ...
  function roll() {
    ...
    setTimeout(() => {
      ...
      const exaustionMod = -char.exaustao;
      const effectBonus = sumSelectedEffectValue(char.effects, selectedEffects, "roll");
      const total = highestDie + config.prog + ef + effectBonus + exaustionMod;
      const autoDt = TIER_DT[config.tier];
      const auto = autoDt && dt <= autoDt;
      const success = auto || total >= dt;
      const threat = highestDie === 6 && !auto;
      const critfail = highestDie === 1 && !success;
      const result = {
        dice,
        highestDie,
        total,
  ...
  return (
    <div className="desktop-split">
      <Sect title="Configurar Rolagem" color={C.gold}>
        ...
        <div style={{ padding: "8px 10px", background: C.bg3, borderRadius: 6, fontSize: 12, marginBottom: 10, display: "flex", flexWrap: "wrap", gap: 8, alignItems: "center" }}>
          ...
          {char.exaustao > 0 ? (
            <>
              <span style={{ color: C.muted }}>+</span>
              <span style={{ color: "#EF4444" }}>{`-${char.exaustao}`} exaustao</span>
            </>
          ) : null}
      ...
      <div>
        <Sect title="Resultado" color={res ? (res.success ? C.green : "#EF4444") : C.muted}>
          {res ? (
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                <div style={{ fontFamily: "Georgia,serif", fontSize: 15, color: res.auto ? "#FDE68A" : res.threat ? C.gold : res.success ? C.green : "#EF4444", fontWeight: 700, letterSpacing: 1 }}>
                  {res.auto ? "AUTO-SUCESSO" : res.threat ? "AMEACA" : res.success ? "SUCESSO" : res.critfail ? "FALHA CRITICA" : "FALHA"}
                </div>
                <div style={{ fontFamily: "Georgia,serif", fontSize: 30, fontWeight: 700, color: res.success ? C.green : "#EF4444", lineHeight: 1 }}>
                  {res.total}
                </div>
              </div>
              ...
              <div style={{ fontSize: 11, color: C.muted }}>
                ...
                {res.exaustionMod !== 0 ? <> exaustao <strong style={{ color: "#EF4444" }}>{res.exaustionMod}</strong></> : null}
                {" = "}
              ...
              {res.threat ? (
                <div style={{ fontSize: 11, color: C.gold, marginTop: 6, padding: "4px 8px", background: `${C.gold}11`, borderRadius: 4 }}>
                  Role novamente vs DT {res.dt} para confirmar o critico.
                </div>
              ) : null}

              {res.critfail ? (
                <div style={{ fontSize: 11, color: "#EF4444", marginTop: 6, padding: "4px 8px", background: "#EF444411", borderRadius: 4 }}>
          ...
          <Sect title="Historico" color={C.muted}>
            {hist.slice(1).map((item, index) => (
              <div key={index} style={{ display: "flex", justifyContent: "space-between", padding: "4px 0", borderBottom: `1px solid ${C.border}`, fontSize: 12 }}>
                ...
                <span style={{ color: item.success ? C.green : "#EF4444", fontWeight: 600 }}>
        ...
        <div>
          ...
          <ResponsiveGrid minWidth={320}>
            {fighters.map((fighter, index) => {
              ...
              const hpColor = percent > 50 ? C.green : percent > 25 ? "#FACC15" : "#EF4444";
              return (
                  ...
                  <div style={{ marginBottom: 6 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 3 }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 3 }}>
                        <SmBtn onClick={() => changeHp(fighter.id, -10)} color="#EF4444" wide>-10</SmBtn>
                        <SmBtn onClick={() => changeHp(fighter.id, -5)} color="#EF4444" wide>-5</SmBtn>
                        <SmBtn onClick={() => changeHp(fighter.id, -1)} color="#EF4444">-</SmBtn>
                    ...
                    {fighter.hp === 0 ? (
                      <div style={{ fontSize: 10, color: "#EF4444", marginTop: 2 }}>Inconsciente - teste de Constituicao DT 5 por turno.</div>
                    ) : null}
                  </div>
...
function ArmasDmgTab({ char, damageBonus }) {
  ...
  function getFormula() {
    ...
    return selWeapon.dmg.split("/")[0].trim();
  }
        ...
        <div>
          ...
          {selWeapon ? (
            <div style={{ padding: "8px 10px", background: C.bg3, borderRadius: 6, marginBottom: 10, fontSize: 11 }}>
              ...
              <div style={{ color: C.muted }}>{selWeapon.type}{selWeapon.range ? ` - ${selWeapon.range}` : ""}{selWeapon.emp ? ` - ${selWeapon.emp}` : ""} - Acao: {selWeapon.act}</div>
              <div style={{ color: "#FACC15", marginTop: 2 }}>Dado: <strong>{selWeapon.dmg}</strong> - Critico: {selWeapon.crit}</div>
            </div>
          ) : null}
        </div>
      ) : (
...
function ManifDmgTab({ char, damageBonus }) {
  ...
  const actionMeta = getActionMeta(amp);
  ...
  return (
    <Sect title="Rolagem de Dano - Manifestacao" color={C.alma}>
      ...
      <div style={{ padding: "8px 10px", background: `${C.alma}11`, border: `1px solid ${C.alma}33`, borderRadius: 6, marginBottom: 12 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
          ...
          <div style={{ textAlign: "right" }}>
            ...
            <div style={{ fontSize: 12, fontWeight: 700, color: actionMeta.color }}>{actionMeta.label.split("-")[0].trim()}</div>
          </div>
        </div>
      ...
      <div style={{ marginTop: 14 }}>
        <div style={{ fontFamily: "Georgia,serif", fontSize: 10, color: C.muted, letterSpacing: 2, marginBottom: 6 }}>Escala rapida - {selTier.toUpperCase()}</div>
        <div className="chip-row">
          {(DMG_ROWS.find((row) => row.tier === selTier) || DMG_ROWS[0]).vals.map((value, index) => (
            <div
              key={index}
              onClick={() => setAmp(AMP_THRESHOLDS[index])}
              style={{
                flex: 1,
                minWidth: 52,
                padding: "5px 4px",
                background: ampBand.index === index ? `${C.alma}22` : C.bg3,
                border: `1px solid ${ampBand.index === index ? C.alma : C.border}`,
                borderRadius: 5,
                textAlign: "center",
                cursor: "pointer",
              }}
            >
              ...
              <div style={{ fontSize: 12, fontWeight: 700, color: ampBand.index === index ? C.alma : C.text }}>{value}</div>
            </div>
          ))}
        </div>
        <div style={{ fontSize: 10, color: C.muted, marginTop: 6 }}>Toque nas colunas para selecionar a faixa de amplificacao.</div>
      </div>
    </Sect>
  );
}
