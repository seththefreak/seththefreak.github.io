const TRACKER_STAGED_CONDITIONS = [
  ...
  { id: "fragilizado", label: "Fragilizado", maxLevel: 3, color: "#F59E0B", note: "Afeta a RD em -1 / -2 / -3. No nivel 3, considere quebra estrutural." },
  { id: "queimando", label: "Queimadura", maxLevel: 2, color: "#FB923C", note: "Escala calor continuo, pressao e negacao de espaco seguro." },
  { id: "congelado", label: "Congelamento", maxLevel: 3, color: "#67E8F9", note: "Escala perda de movimento e abre janela para dano fisico ampliado." },
  { id: "envenenado", label: "Envenenado", maxLevel: 3, color: "#4ADE80", note: "Escala desgaste corporal, testes piores e dano continuo." },
];

const TRACKER_BINARY_CONDITIONS = [
  { id: "atordoado", label: "Atordoado", color: "#FACC15", note: "Perde a proxima Acao Maior." },
  { id: "derrubado", label: "Derrubado", color: "#FB923C", note: "-2 Defesa. Levantar exige gasto de utilidade / menor." },
  { id: "imobilizado", label: "Imobilizado", color: "#F97316", note: "Sem movimento e -2 Destreza ate sair do controle." },
];
...
function buildTrackerState(char, fighter, existing) {
  ...
  return {
    ...
    defenseBase: Math.max(0, Number(current.defenseBase) || 0),
    rdCurrent: Math.max(0, Number(current.rdCurrent) || 0),
    ...
    channelPe: Math.max(0, Number(current.channelPe) || 0),
    stages,
  };
}
...
function getOverdraftState(peValue) {
  ...
  if (overdraft <= 5) {
    return { amount: overdraft, label: "Leve", color: C.mente, note: "Recuperacao longa cai para metade." };
  }
  if (overdraft <= 10) {
...
function getDefenseSnapshot(char, fighter, tracker, band) {
  ...
  return {
    defenseCurrent: Math.max(0, tracker.defenseBase + band.defensePenalty),
    rdFinal: Math.max(0, tracker.rdCurrent - fragStage),
    fragStage,
