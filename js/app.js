...
<link href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,300;0,400;0,600;0,700;1,400&family=Nunito:wght@300;400;600;700&display=swap" rel="stylesheet"/>
<style>
  body{font-family:'Nunito',sans-serif;background:#f8f4ff;color:#3d3452;padding:40px;max-width:720px;margin:0 auto;}
  h1{font-family:'Cormorant Garamond',serif;font-size:2.4rem;color:#8060b8;margin:0 0 8px;}
  h2{font-family:'Cormorant Garamond',serif;font-size:1.1rem;color:#8060b8;border-bottom:1px solid #ddd4f0;padding-bottom:6px;margin:20px 0 10px;}
  ...
  .attr-v{font-family:'Cormorant Garamond',serif;font-size:1.3rem;font-weight:700;}
...
function calcHP(sheet) {
  const LV   = sheet.nivel || 1;
  const VIDA = evSheet(sheet, 'VIDA');
  const CON  = evSheet(sheet, 'CON');
  return Math.round(
    30
    + 0.7 * _safe(VIDA)**2 + 2*_safe(VIDA) + 4*_pen(VIDA)
    + 0.5 * _safe(CON)**2  + 2*_safe(CON)  + 3*_pen(CON)
    + (LV-1) * (2*_safe(CON) + _safe(VIDA))
  );
}
...
function calcMP(sheet) {
  const LV  = sheet.nivel || 1;
  const MAG = evSheet(sheet, 'MAG');
  const INT = evSheet(sheet, 'INT');
  return Math.round(
    20
    + 0.6 * _safe(MAG)**2 + 2*_safe(MAG) + 3*_pen(MAG)
    + 0.4 * _safe(INT)**2 + 2*_safe(INT) + 2*_pen(INT)
    + (LV-1) * (_safe(MAG) + _safe(INT))
  );
}
...
function calcSAN(sheet) {
  const LV  = sheet.nivel || 1;
  const VON = evSheet(sheet, 'VON');
  const SAB = evSheet(sheet, 'SAB');
  return Math.round(
    25
    + 0.8 * _safe(VON)**2 + 2*_safe(VON) + 3*_pen(VON)
    + SAB
    + (LV-1) * _safe(VON)
  );
}
...
function calcMV(sheet) {
  const LV  = sheet.nivel || 1;
  const AGI = evSheet(sheet, 'AGI');
  return _fl(4 + AGI + _fl(3*_ln(_safe(AGI)+1)/_ln(11)) + LV/6);
}
...
function calcEV(sheet) {
  const LV  = sheet.nivel || 1;
  const DES = evSheet(sheet, 'DES');
  const AGI = evSheet(sheet, 'AGI');
  return _fl(DES + _fl(4*_ln(_safe(AGI)+1)/_ln(11)) + LV/6);
}
...
function calcBLK(sheet) {
  return _fl((evSheet(sheet,'FOR') + evSheet(sheet,'DES')) / 10);
}
...
function calcDEF_TOTAL(sheet) {
  const CON  = evSheet(sheet, 'CON');
  const FOR  = evSheet(sheet, 'FOR');
  const DEFb = evSheet(sheet, 'DEF');
  const equip = _getEquipBonus(sheet, 'ca');
  return DEFb + _fl((_safe(CON) + _safe(FOR)) / 6) + equip;
}
...
function calcMR_TOTAL(sheet) {
  const VON   = evSheet(sheet, 'VON');
  const CON   = evSheet(sheet, 'CON');
  const RESMb = evSheet(sheet, 'RESM');
  const equip = _getEquipBonus(sheet, 'rm');
  return RESMb + _fl((_safe(VON) + _safe(CON)) / 6) + equip;
}
...
function calcATK_TOTAL(sheet) {
  const ATQb = evSheet(sheet, 'ATQ');
  const DES  = evSheet(sheet, 'DES');
  const FOR  = evSheet(sheet, 'FOR');
  const { w, p } = _getWeaponFactors(sheet);
  const inner = _safe((1-p)*DES + p*FOR);
  return Math.round(ATQb + w * _fl(5*_ln(inner+1)/_ln(11)));
}
...
function calcPWR(sheet) {
  const MAG = evSheet(sheet, 'MAG');
  const INT = evSheet(sheet, 'INT');
  return MAG + _fl(5*_ln(_safe(MAG+INT)+1)/_ln(11));
}
...
function calcWIL(sheet) {
  ...
  const VON = evSheet(sheet, 'VON');
  const CON = evSheet(sheet, 'CON');
  return _fl(VON + _fl(3*_ln(_safe(VON+CON)+1)/_ln(11)) + LV/6);
}
...
function calcPER_TESTE(sheet) {
  const PER = evSheet(sheet, 'PER');
  const SAB = evSheet(sheet, 'SAB');
  const AGI = evSheet(sheet, 'AGI');
  return PER + _fl(3*_ln(_safe(SAB+AGI)+1)/_ln(11));
}
...
function calcLK(sheet) {
  // LK = 10 + CAR + floor(SAB/2) + racial LK bonus
  var raceLKBonus = 0;
  if (sheet && sheet.raceSelecionada) {
    var race = RACES_DB.find(function(r){ return r.name === sheet.raceSelecionada; });
    raceLKBonus = (race && race.atributos && race.atributos.LK) ? race.atributos.LK : 0;
  }
  return 10 + evSheet(sheet,'CAR') + _fl(evSheet(sheet,'SAB')/2) + raceLKBonus;
}

/* ── HELPERS DE EQUIPAMENTO ─────────────────────────────── */
function _getEquipBonus(sheet, campo) {
  if (!sheet.equipamento || typeof EQUIPAMENTOS_INDEX === 'undefined') return 0;
  return Object.values(sheet.equipamento).reduce((sum, itemId) => {
    if (!itemId) return sum;
    // Skip locked slots (two-handed weapon marker)
    if (typeof itemId === 'string' && itemId.endsWith('__bloqueado')) return sum;
    const item = EQUIPAMENTOS_INDEX.get(itemId);
    return sum + (item?.[campo] ?? 0);
  }, 0);
}
function _getWeaponFactors(sheet) {
  if (!sheet.equipamento || typeof EQUIPAMENTOS_INDEX === 'undefined') return { w:0.5, p:0.5 };
  let armaId = sheet.equipamento.maoDireita || sheet.equipamento.maoEsquerda;
  // Strip __bloqueado suffix if present
  if (typeof armaId === 'string' && armaId.endsWith('__bloqueado')) armaId = armaId.replace('__bloqueado', '');
  if (!armaId) return { w:0.5, p:0.5 };
  const arma = EQUIPAMENTOS_INDEX.get(armaId);
  if (!arma) return { w:0.5, p:0.5 };
  const wMap = { leve:0.4, media:0.7, pesada:1.0 };
  const pMap = { Impacto:0.85, Corte:0.55, Perfuração:0.35, Perfuro:0.35, Mágico:0.0, Laceração:0.6 };
  return { w: wMap[arma.categoria]??0.5, p: pMap[arma.tipoDano]??0.5 };
}
...
function calcularTodosDerivados(sheet) {
  return {
    ...
    EV:        calcEV(sheet),
    BLK:       calcBLK(sheet),
    DEF_TOTAL: calcDEF_TOTAL(sheet),
    MR_TOTAL:  calcMR_TOTAL(sheet),
    ATK_TOTAL: calcATK_TOTAL(sheet),
    PWR:       calcPWR(sheet),
    WIL:       calcWIL(sheet),
    PER_TESTE: calcPER_TESTE(sheet),
    LK:        calcLK(sheet),
  };
}
...
function calcularSubatributos(sheet) {
  ...
  return {
    // campos usados pelo código legado (compacto/expandido/radar)
    VIDA:   d.HP,
    DEF:    d.DEF_TOTAL,
    ATQ:    d.ATK_TOTAL,
    RESM:   d.MR_TOTAL,
...
.res-bar-cur{font-family:'Cormorant Garamond',serif;font-size:1.2rem;font-weight:700;color:var(--text-primary);transition:color .3s;}
...
function renderFichaExpandida() {
    ...
    const rows = g.items.map(({val, label, influence, desc}) => {
      ...
      return `<div class="expanded-hab-item" style="padding:10px 14px;margin-bottom:6px;">
        <div style="display:flex;justify-content:space-between;align-items:center;">
          ...
          <div style="font-family:'Cormorant Garamond',serif;font-size:1.4rem;font-weight:700;color:var(--text-accent);">${sign}${val}</div>
        </div>
  ...
  const modosHTML = Object.entries(modos).map(([k,v]) =>
    `<div class="read-modo ${v===maxM&&v>1?'dominant':''}">
      <div style="display:flex;justify-content:space-between;align-items:center;">
        ...
        <strong style="font-family:'Cormorant Garamond',serif;font-size:1.2rem;">${v}</strong>
      </div>
    ...
    if (caminho) {
      ...
      caminhoBlock = `
        <div style="background:var(--lavanda-light);border:1.5px solid var(--lavanda);border-radius:var(--radius-md);padding:12px 16px;margin-bottom:14px;">
          <div style="font-family:'Cormorant Garamond',serif;font-size:1rem;font-weight:700;color:var(--text-accent);margin-bottom:4px;">Caminho: ${escapeHtml(caminho.nome)}</div>
  ...
  const periciaRows = Object.entries(pericias).filter(([,v]) => v !== 0).map(([nome, val]) =>
    `<div style="display:flex;justify-content:space-between;align-items:center;padding:5px 0;border-bottom:1px solid var(--border-light);font-size:0.85rem;">
      ...
      <span style="font-family:'Cormorant Garamond',serif;font-size:1.05rem;font-weight:700;color:${val>0?'var(--attr-pos)':'var(--attr-neg)'};">${val>0?'+':''}${val}</span>
    </div>`).join('');
  ...
  container.innerHTML = `
    <div class="expanded-sheet">
      ...
      <h1 style="font-family:'Cormorant Garamond',serif;font-size:2rem;font-weight:700;color:var(--text-accent);text-align:center;margin-bottom:6px;">${escapeHtml(sheet.name || 'Sem nome')}</h1>
      ...
      ${sheet.conceitoNarrativo ? `<div style="font-style:italic;text-align:center;color:var(--text-secondary);font-size:0.95rem;line-height:1.7;font-family:'Cormorant Garamond',serif;margin-bottom:24px;">"${escapeHtml(sheet.conceitoNarrativo)}"</div>` : ''}
